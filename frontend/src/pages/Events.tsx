...
const Events: React.FC = () => {
  ...
  const fetchEvents = async () => {
    try {
      ...
      if (err.response?.status !== 401) {
        setError(t('errors.failedLoadEvents') || 'Failed to load events. Please try again later.');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleJoinEvent = async (eventId: number) => {
    try {
      ...
      // опционально можно показать тост с t('errors.failedJoin')
    }
  };
  ...
  const handleLeaveEvent = async (eventId: number) => {
    try {
      ...
      // опционально можно показать тост с t('errors.failedLeave')
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {error && (
        <div className="bg-red-50 border border-red-400 text-red-700 px-4 py-3 rounded mb-6">
          {error}
          <button onClick={fetchEvents} className="ml-4 text-red-800 underline hover:no-underline">
            {t('common.tryAgain')}
          </button>
        </div>
      )}
      ...
      {events.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-gray-500 text-lg">{t('events.none')}</p>
        </div>
      ) : (
        <>
                  ...
                  <div className="p-6">
                    ...
                    <div className="space-y-2 text-sm text-gray-500 mb-4">
                      <div className="flex items-center">
                        ...
                        {t('events.dateAt', { date: dateStr, time: timeStr })}
                      </div>
                      {event.location && (
                        <div className="flex items-center">
                          <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <div className="flex items-center">
                        ...
                        {(event.participantCount || 0)} {t('events.participants')}
                      </div>
                      <div className="flex items-center">
                        <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        </svg>
                        {t('events.createdBy', { name: event.creatorName || 'Unknown' })}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="px-6 pb-6">
                    ...
                    <div className="mt-3">
                      <button
                        ...
                        className="btn-secondary w-full"
                      >
                        {openReviewsForEventId === event.id ? t('events.hideReviews') : t('events.viewReviews')}
                      </button>
                    </div>
                  </div>
          ...
          <div className="mt-6 flex items-center justify-center gap-2">
            ...
            <span className="text-sm text-gray-700">{t('usersList.page', { page, total: totalPages })}</span>
            <button
              className="btn-secondary"
